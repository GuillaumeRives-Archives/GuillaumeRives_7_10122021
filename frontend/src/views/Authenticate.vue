<template>
   <main class="container text-center">
      ...
      <p class="fs-4">Authentifiez-vous afin de partager du contenu multimédia avec vos collaborateurs !</p>
      <div class="col-md-9 m-auto mt-4">
         ...
         <div class="tab-content" id="AuthenticateTabsContent">
            <div
               class="tab-pane fade show active border-start border-end border-bottom bg-white rounded-bottom shadow-sm text-center"
               id="login"
               role="tabpanel"
            ...
            >
               ...
               <div v-if="login_errors.length" class="mt-2 mb-4 ms-2 me-2">
                  <span v-for="error in login_errors" :key="error.index" class="alert" :class="login_error_type"
                     ><i class="bi bi-exclamation-triangle-fill"></i> {{ error }}</span
                  >
               </div>
            </div>
            <div
               class="tab-pane fade border-start border-end border-bottom bg-white rounded-bottom shadow-sm text-center"
               id="register"
               role="tabpanel"
            ...
            >
               <form action="POST" class="row p-4">
                  ...
                  <div class="col-md-7 m-auto text-start">
                     ...
                     <input
                        v-model="register_password_check"
                        type="password"
               ...
               <div v-if="register_errors.length" class="mt-2 mb-4 ms-2 me-2">
                  <span v-for="error in register_errors" :key="error.index" class="alert" :class="register_error_type"
                     ><i class="bi bi-exclamation-triangle-fill"></i> {{ error }}</span
                  >
               </div>
            </div>
      ...
      <div class="mt-3">
         Un problème pour vous authentifier ?
         <a class="link-primary" target="blank" href="mailto:contact@groupomania.com">contactez nous !</a>
      </div>
   </main>
</template>
      ...
      methods: {
         login() {
            ...
            if (mailRegex.test(this.login_email) && this.login_password !== "") {
               ...
               this.login_errors.push("Merci de renseigner tous les champs correctement");
            }
         },
            ...
            if (mailRegex.test(this.register_email)) {
               if (this.register_password !== "" && this.register_password_check !== "") {
                  if (this.register_password === this.register_password_check) {
                     ...
                     this.register_errors.push("Les deux mots de passe ne correspondent pas...");
                  }
               } else {
                  this.register_error_type = "alert-warning";
                  this.register_errors.push("Merci de renseigner les champs de mot de passe...");
               }
            } else {
               this.register_error_type = "alert-warning";
               this.register_errors.push("Veuillez vérifier le format de votre adresse email...");
            }
         },
      },
   };
